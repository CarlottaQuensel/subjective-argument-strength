...
if __name__ == '__main__':
    ...
    parser.add_argument("variable", type=str)  # mixed or cmv_only
    parser.add_argument("num_splits", type=int)
    parser.add_argument("epochs", type=int)
    args = parser.parse_args()

    # Make output and temp folders if they don't exist
    dest_folder = "results/storytelling/" + args.variable
    ...
    storytelling_data = pd.read_csv(
        f"data/storytelling/storytelling_{args.variable}.csv", sep="\t")
    print("Loaded training data from <==",
          f"data/storytelling/storytelling_{args.variable}.csv")
    ...
    for split in range(num_splits):
        ...
        tmp = {"text_id": y_ids, f"prediction_{split}": y_pred, f"score_{split}": y_scores}
    ...
    ibm_splits.drop(columns=(f"score_{s}" for s in range(num_splits)), inplace=True)
    cmv_splits.drop(columns=(f"score_{s}" for s in range(num_splits)), inplace=True)
    ...
    ibm_df.to_csv(
        dest_folder+f"/ibm_with_storytelling_{args.variable}.csv", sep="\t")
    cmv_df.to_csv(
        dest_folder+f"/cmv_with_storytelling_{args.variable}.csv", sep="\t")
